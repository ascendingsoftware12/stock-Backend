...
# --------------------SALES--------------------
def search_PeriodComparison_controller():
    try:
...
# -------------------DISCOUNT AMOUNT--------------------------
def search_PeriodComparison_DisAmt_controller():
    try:
        period1_from = request.args.get("period1_from")
        period1_to = request.args.get("period1_to")
        period2_from = request.args.get("period2_from")
...
# -------------------SALES QUENTITY--------------------------
def search_PeriodComparison_SalesQty_controller():
    try:
        period1_from = request.args.get("period1_from")
        period1_to = request.args.get("period1_to")
        period2_from = request.args.get("period2_from")
...
# -------------------DISCOUNT%--------------------------
def search_PeriodComparison_dis_controller():
    try:
...
# -------------------STORE CODE--------------------------
def search_PeriodComparison_store_code_controller():
    try:
        # Fetch parameters
        period1_from = request.args.get("period1_from")
        period1_to = request.args.get("period1_to")
        ...
        query = db.session.query(
            func.count(
                case(
                    (SalesAllInOneLive.invoice_date.between(period1_from, period1_to), SalesAllInOneLive.store_code),
                    else_=None
                )
            ).label("total_period1_sales"),
            func.count(
                case(
                    (SalesAllInOneLive.invoice_date.between(period2_from, period2_to), SalesAllInOneLive.store_code),
                    else_=None
                )
            ).label("total_period2_sales"),
            SalesAllInOneLive.store_code  # Add group by field
        ).filter(*conditions).group_by(SalesAllInOneLive.store_code)

        # Execute query
        result = query.all()

        # Process result and calculate growth percentage
        result_data = []
        total_period1_sales = 0
        total_period2_sales = 0
        total_period1_count = 0  # Initialize count variables
        total_period2_count = 0  # Initialize count variables
        overall_growth_percentage = None  # Default value

        for row in result:
            period1_sales = row.total_period1_sales
            period2_sales = row.total_period2_sales

            # Accumulate totals
            total_period1_sales += period1_sales
            total_period2_sales += period2_sales

            # Count total sales for both periods
            if period1_sales > 0:
                total_period1_count += 1
            if period2_sales > 0:
                total_period2_count += 1

            # Calculate growth percentage (from SQL logic)
            if total_period1_count != 0:
                growth_percentage = ((total_period2_count - total_period1_count) / total_period1_count) * 100
            else:
                growth_percentage = None

            # Keep track of the last calculated growth percentage
            overall_growth_percentage = growth_percentage

        # Append totals to the result
        overall_totals = {
            "growth_percentage": overall_growth_percentage,
            "total_period1_count": total_period1_count,  # Add count for period 1
            "total_period2_count": total_period2_count   # Add count for period 2
        }

        # Return response
        return jsonify({"success": 1, "overall_totals": overall_totals})

    except Exception as e:
        # Rollback transaction on error
        db.session.rollback()
        if "MySQL server has gone away" in str(e):
            return search_PeriodComparison_store_code_controller()  # Retry logic
        else:
            return jsonify({"success": 0, "error": str(e)})

# -------------------ASP--------------------------
def search_PeriodComparison_asp_controller():
    try:
        ...
        db.session.rollback()
        if "MySQL server has gone away" in str(e):
            return search_PeriodComparison_asp_controller()  # Retry logic
        else:
            return jsonify({"success": 0, "error": str(e)})

# -------------------BRANCH WISE ANALYSIS--------------------------
def search_PeriodComparison_branch_wise_Analysis_controller():
    try:# Fetch additional filters from request arguments
        section = request.args.get('section')
        sales_type = request.args.get('sales_type')
        item_category = request.args.get('item_category')
        product_group = request.args.get('product_group')
        ...
        for row in result:
            result_data.append({
                ...
                "period1_total_sales": row.period1_total_sales,
                "period2_total_sales": row.period2_total_sales,
        ...
        if "MySQL server has gone away" in str(e):
            ...
            return jsonify({"success": 0, "error": str(e)})
# -------------------CITY WISE ANALYSIS--------------------------
def search_PeriodComparison_city_wise_Analysis_controller():
    try:# Fetch additional filters from request arguments
        section = request.args.get('section')
        sales_type = request.args.get('sales_type')
        item_category = request.args.get('item_category')
        ...
        period1_city_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period1_from, period1_to), total_sales_field),
                else_=0
            )
        ).label('period1_city_total_sales')

        period2_city_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period2_from, period2_to), total_sales_field),
                else_=0
            )
        ...
        query = db.session.query(
            ...
            period1_city_total_sales,
            period2_city_total_sales,
            growth_percentage
        ).filter(*conditions
        ...
        for row in result:
            result_data.append({
                "city": row.city,
                "period1_city_total_sales": row.period1_city_total_sales,
                "period2_city_total_sales": row.period2_city_total_sales,
                "growth_percentage": row.growth_percentage
            })
...
def search_PeriodComparison_section_wise_Analysis_controller():
    try:
...
# -------------------ITEMCATEGORY WISE ANALYSIS--------------------------
def search_PeriodComparison_itemcategory_wise_Analysis_controller():
    ...
    try:
        ...
        period1_itemcategory_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period1_from, period1_to), total_sales_field),
                else_=0
            )
        ).label('period1_itemcategory_total_sales')

        period2_itemcategory_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period2_from, period2_to), total_sales_field),
                else_=0
            )
        ...
        query = db.session.query(
            ...
            period1_itemcategory_total_sales,
            period2_itemcategory_total_sales,
            growth_percentage
        ...
        for row in result:
            result_data.append({
                ...
                "period1_itemcategory_total_sales": row.period1_itemcategory_total_sales,
                "period2_itemcategory_total_sales": row.period2_itemcategory_total_sales,
                "growth_percentage": row.growth_percentage
            })
...
def search_PeriodComparison_product_wise_Analysis_controller():
    ...
    try:
        ...
        period1_product_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period1_from, period1_to), total_sales_field),
                else_=0
            )
        ).label('period1_product_total_sales')

        period2_product_total_sales = func.sum(
            case(
                (SalesAllInOneLive.invoice_date.between(period2_from, period2_to), total_sales_field),
                else_=0
            )
        ...
        for row in result:
            result_data.append({
                ...
                "period1_product_total_sales": row.period1_product_total_sales,
                "period2_product_total_sales": row.period2_product_total_sales,
                "growth_percentage": row.growth_percentage
            })
...
# -------------------BRAND WISE ANALYSIS--------------------------
def search_PeriodComparison_brand_wise_Analysis_controller():
...
# -------------------PRICE WISE ANALYSIS--------------------------
def search_PeriodComparison_price_wise_Analysis_controller():
    try:
        # Retrieve query parameters
        period1_from = request.args.get("period1_from")
        period1_to = request.args.get("period1_to")
