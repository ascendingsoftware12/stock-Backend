...
def calculate_approved_procurement_fields(stock):
    try:
        ...
        db.session.rollback()
        if "MySQL server has gone away" in str(e):
            return calculate_approved_procurement_fields(stock)
...
def calculate_total_yet_to_procure_projected_qty_for_approved():
    try:
        ...
        db.session.rollback()
        if "MySQL server has gone away" in str(e):
            return calculate_total_yet_to_procure_projected_qty_for_approved()
...
def update_projection_procurement_controller():
    try:
        ...
        approved_records = MStockOptimizationModel.query.filter(MStockOptimizationModel.p_approved_flag == "TRUE", MStockOptimizationModel.p_final_po_qty != None, MStockOptimizationModel.p_final_po_flag == "TRUE" ,MStockOptimizationModel.po_number == None).all()
