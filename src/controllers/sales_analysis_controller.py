...
def search_SalesContributuion_section_controller():
    try:
        ...
        ).filter(*conditions).group_by(SalesAllInOneLive.section).limit(limit).offset(offset)
        ...
        for row in section_result:
            ...
            total_sales_percentage = (row.total_sales / total_sales_sum) * 100 if total_sales_sum else 0
            total_sales_percentage_str = f"{total_sales_percentage:.2f}%"

            section_result_data.append({
                "section": row.section,
                "total_sales": int(row.total_sales),
                "asp": int(asp),
                "total_sales_percentage":total_sales_percentage_str
            })
...
def search_SalesContributuion_item_category_controller():
    try:
        ...
        item_category_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.item_category).limit(limit).offset(offset)

        
        item_category_result = item_category_query.all()

        
        total_sales_sum = sum(row.total_sales for row in item_category_result)
...
def search_SalesContributuion_product_controller():
    try:
        ...
        product_group_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.product_group).limit(limit).offset(offset)

        
        product_group_result = product_group_query.all()

        
        total_sales_sum = sum(row.total_sales for row in product_group_result)
        item_category_result_data = []
...
def search_SalesContributuion_branch_controller():
    try:
        ...
        store_name_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.store_name).limit(limit).offset(offset)

        
        store_name_result = store_name_query.all()

        
        total_sales_sum = sum(row.total_sales for row in store_name_result)
        sales_qty_sum = sum(row.sales_qty for row in store_name_result)
        ...
        for row in store_name_result:
            ...
            total_sales_percentage = (row.total_sales / total_sales_sum) * 100 if total_sales_sum else 0
            total_sales_percentage_str = f"{total_sales_percentage:.2f}%"
            sales_qty_percentage = (row.sales_qty / sales_qty_sum) * 100 if sales_qty_sum else 0
            sales_qty_percentage_str = f"{sales_qty_percentage:.2f}%"

            store_name_result_data.append({
                "store_name": row.store_name,
                "total_sales": int(row.total_sales),  
                "sales_qty": int(row.sales_qty),      
                "sales_qty_percentage": sales_qty_percentage_str,
                "asp": asp,
                "total_sales_percentage": total_sales_percentage_str
            })
...
def search_SalesContributuion_city_controller():
    try:
        ...
        city_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.city).limit(limit).offset(offset)
        ...
        sales_qty_sum = sum(row.sales_qty for row in city_result)
        city_result_data = []
...
def search_SalesContributuion_brand_sales_controller():
    try:
        ...
        brand_name_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.brand_name).limit(limit).offset(offset)

        
        brand_name_result = brand_name_query.all()

        
        total_sales_sum = sum(row.total_sales for row in brand_name_result)
        sales_qty_sum = sum(row.sales_qty for row in brand_name_result)
...
def search_SalesContributuion_item_sales_controller():
    try:
        ...
        actual_item_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.sales_qty).label('sales_qty')
        ).filter(*conditions).group_by(SalesAllInOneLive.actual_item).limit(limit).offset(offset)

        
        actual_item_result = actual_item_query.all()

        
        total_sales_sum = sum(row.total_sales for row in actual_item_result)
        sales_qty_sum = sum(row.sales_qty for row in actual_item_result)
...
def search_Discount_Analysis_branch_controller():
    try:
        ...
        branch_result = store_name_query.all()
...
def search_Discount_Analysis_model_no_controller():
    try:
        ...
        model_no_query = db.session.query(
            ...
            func.sum(SalesAllInOneLive.total_sales).label('total_sales'),
            func.sum(SalesAllInOneLive.disc_amt).label('disc_amt')
        ).filter(*conditions).group_by(SalesAllInOneLive.model_no).limit(limit).offset(offset)

        
        model_no_result = model_no_query.all()
