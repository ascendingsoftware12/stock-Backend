...
def get_sales_all_in_one_live_kore_ytd_cr_controller():
    try:
...
def get_sales_all_in_one_live_kore_month_cr_controller():
    try:
...
def get_sales_all_in_one_live_kore_weekly_analysis_cr_controller():
    try:
...
ORDER BY fiscal_year, fiscal_week_number;
        ...
        # Group weeks by fiscal year
        weekly_sales_by_fiscal_year = defaultdict(list)
        for row in weekly_sales:
            week_number, week_start, month, week_end, fiscal_year, total_sales, sales_qty, tax_amt, gros_profit = row
            weekly_sales_by_fiscal_year[fiscal_year].append(row)

        # Assign fiscal week numbers correctly
        fiscal_week_mapping = {}
        for fiscal_year, weeks in weekly_sales_by_fiscal_year.items():
            sorted_weeks = sorted(weeks, key=lambda x: x[0])  # Sort by week_number
            fiscal_week_mapping[fiscal_year] = {
                week[0]: i + 1 for i, week in enumerate(sorted_weeks)
            }

        # Process each row and compute fiscal_week_number
        for row in weekly_sales:
            week_number, week_start, month, week_end, fiscal_year, total_sales, sales_qty, tax_amt, gros_profit = row
            fiscal_years = fiscal_year
            fiscal_week_number = fiscal_week_mapping[fiscal_year][week_number]
            ...
            week_label = f"Week {fiscal_week_number:02}"
...
def get_sales_all_in_one_live_kore_day_analysis_cr_controller():
    try:
...
def get_sales_all_in_one_live_kore_brand_dimension_cr_controller():
    try:
        period_from = request.args.get("period_from")
        period_to = request.args.get("period_to")
        invoice_date = request.args.get('invoice_date')
...
def get_sales_all_in_one_live_kore_item_dimension_cr_controller():
    try:
...
#         else:
#             return jsonify({"success": 0, "error": str(e)})
def get_sales_all_in_one_live_kore_price_breakup_one_cr_controller():
    try:
...
def get_sales_all_in_one_live_kore_price_breakup_two_cr_controller():
    try:
...
def get_sales_all_in_column_kore_live_controller():
    try:
        period_from = request.args.get("period_from")
        period_to = request.args.get("period_to")
        invoice_date = request.args.get('invoice_date')
...
def get_sales_all_in_kore_date_controller():
    try:
        store_code = request.args.get('storecode')
        ...
        # Query to get the min and max invoice_date from the sales_all_in_one_live table with conditions
        query = db.session.query(
...
def get_sales_all_in_one_live_kore_section_dimension_cr_controller():
    try:
        # Get request parameters
        factor = request.args.get('gstfilter', 'cr')  # Default to 'cr'
        period_from = request.args.get("period_from")
...
def get_sales_all_in_one_live_kore_table_modification_date_and_time_controller():
    try:
